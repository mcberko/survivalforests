...
find_quantile.orsf <- function(surv, max_value, tau){
  b_size <- dim(surv)[1]
  #event.info <- get.event.info(surv)
  bin <- sort(train$time[train$status==1])
  quantiles <- rep(0, b_size)
  actual.max.quant <- rep(0, b_size)
  for (i in 1:b_size){
    new <- train[order(train$time),]
    cens <- which(new$status==0)
    surv.survival <- surv[-cens,]
    check <- surv[i,] >= 1-tau
    j <- max(sum(check), 1)
    quantile <- bin[j]
    quantiles[i] <- quantile
    actual.max.quant[i] <- 1-surv[i,sum(!is.na(bin))]
  }
